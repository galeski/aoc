...
for (let i = 0; i < gamesArray.length; i++) {
  subArray = gamesArray[i];
  subArray = subArray.replace(/.*:/, "");
  subArray = subArray.split(";"); //.split(",");

  if (subArray.every(checkSubarray)) sum += i + 1;
}

function checkSubarray(arr) {
  let splitted = arr.split(",");

  // we can check the last character in a subarray and tell immediately which color it is
  let nums = {
    d: 12,
    e: 14,
    n: 13,
  ...
  for (let elem of splitted) {
    let currColorMax = nums[elem.at(-1)];
    let currNum = elem.match(/\d+/)[0]; // and then match the number using regex

    if (currColorMax < currNum) return false;
  }
