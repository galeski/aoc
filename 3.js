...
// make the data more sane(TM), so that I don't need to deal with undefined
...
lines.unshift(new Array(140).fill("."));
lines.push(new Array(140).fill("."));

let sum = 0;

for (let row = 1; row < lines.length - 1; row++) {
  for (let col = 1; col < lines[row].length - 1; col++) {
    if (!isNaN(lines[row][col])) {
      let numPositions = [col - 1];

      while (!isNaN(lines[row][col])) {
        numPositions.push(col);
        col++;
      }
      ...
      for (let i = 1; i < numPositions.length; i++)
        possibleNumber += lines[row][numPositions[i]];

      for (let i = 1; i < numPositions.length; i++)
        lines[row][numPositions[i]] = ".";
      ...
      if (!allPeriods) sum += Number(possibleNumber);
    }
  }
}
