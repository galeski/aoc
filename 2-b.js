const input = `Game 1: 9 red, 2 green, 13 blue; 10 blue, 2 green, 13 red; 8 blue, 3 red, 6 green; 5 green, 2 red, 1 blue
...
let gamesArray = input.split(/\r?\n/);
let sumPow = 0;

for (let i = 0; i < gamesArray.length; i++) {
  let subArray = gamesArray[i];
  subArray = subArray.replace(/.*:/, "");
  subArray = subArray.split(";"); //.split(",");

  let colors = [0, 0, 0]; // red, green, blue

  checkSubarray(subArray, colors);
  sumPow += colors[0] * colors[1] * colors[2];
}

function checkSubarray(subArray, colors) {
  for (let arr of subArray) {
    let splitted = arr.split(",");

    for (let elem of splitted) {
      let currNum = elem.match(/\d+/)[0];

      if (elem.endsWith("d")) colors[0] = Math.max(currNum, colors[0]); // red
      if (elem.endsWith("n")) colors[1] = Math.max(currNum, colors[1]); // green
      if (elem.endsWith("e")) colors[2] = Math.max(currNum, colors[2]); // blue
    }
  }
}
