...
for (let line of str.split("\n")) {
  let firstNumFromLeft;
  let firstNumFromRight;

  for (let char of line) {
    if (!isNaN(char)) {
      firstNumFromLeft = char;
      break;
    }
  }

  for (let char = line.length - 1; char >= 0; char--) {
    if (!isNaN(line[char])) {
      firstNumFromRight = line[char];
      break;
    }
  }

  sum += parseInt(firstNumFromLeft + firstNumFromRight);
}
